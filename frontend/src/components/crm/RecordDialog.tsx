...
import { shouldTitleCaseField, handleTitleCaseChange } from '@/lib/titleCase';
  ...
  useEffect(() => {
    ...
    if (savedDraft) {
      try {
        ...
        if (resource === 'jobs' && !initial && draftValues.rateType === 'C2C') {
          delete draftValues.rateType;
    ...
    for (const field of fields) {
      if (field.key === 'ownerName') {
        const existingOwner = initial?.ownerName ? String(initial.ownerName) : '';
        next[field.key] = existingOwner || currentUser?.name || 'Administrator';
        continue;
      }
      if (resource === 'jobs' && field.key === 'rateType' && !initial) {
        next[field.key] = '';
        continue;
      }
                ...
                <label key={field.key} className={field.type === 'textarea' || field.type === 'file' ? 'sm:col-span-2' : 'sm:col-span-1'}>
                  ...
                  {field.type === 'select' ? (
                    ...
                    <textarea
                      ...
                      className="crm-input disabled:cursor-not-allowed disabled:bg-[#F7FAFC] disabled:text-slate-400 min-h-24 resize-y py-2 disabled:pointer-events-none select-none"
                      placeholder={field.placeholder}
                      value={values[field.key] ?? ''}
                      onChange={(event) => {
                        const enableTitleCase = shouldTitleCaseField(field);
                        handleTitleCaseChange(event, (val) => updateValue(field.key, val), enableTitleCase);
                      }}
                    />
                  ) : field.type === 'date' ? (
                    ...
                    <input
                      ...
                      className={field.readOnly
                        ? "crm-input bg-[#F7FAFC] text-[#071B4A] font-semibold cursor-not-allowed select-none border-[#E4ECF3]"
                        : "crm-input disabled:cursor-not-allowed disabled:bg-[#F7FAFC] disabled:text-slate-400 disabled:pointer-events-none select-none"
                      }
                      placeholder={field.placeholder}
                      value={values[field.key] ?? ''}
                      onChange={(event) => {
