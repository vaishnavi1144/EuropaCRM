...
import { ArrowDown, ArrowUp, CalendarDays, Download, Edit3, Eye, FileText, MoreVertical, Sparkles, Star, Trash2, X } from 'lucide-react';
        ...
        <table className={`w-full table-auto border-collapse ${onAutomation ? 'min-w-[1420px]' : fitToContainer ? 'min-w-[1120px]' : 'min-w-[1080px]'}`}>
          <thead className="bg-[#fcfdfd]">
            <tr className="border-b border-slate-200">
              ...
              <th className="crm-table-head whitespace-nowrap px-3 py-3 text-center">View Form</th>
              <th className="crm-table-head w-44 min-w-44 px-3 py-3 text-center">Actions</th>
            </tr>
          </thead>
          <tbody>
            {rows.length === 0 ? (
              <tr><td colSpan={columns.length + 3} className="px-5 py-16 text-center"><div className="text-sm font-semibold text-slate-700">No records found</div><div className="mt-1 text-xs text-slate-500">Add a record or change the current search and filters.</div></td></tr>
            ) : rows.map((row) => (
              <tr key={row.id} onClick={() => onRowClick?.(row)} className={`border-b border-slate-100 transition hover:bg-[#fbfefd] last:border-0 ${onRowClick ? 'cursor-pointer' : ''} ${selectedRowId === row.id ? 'bg-[#E6F8F4]' : ''}`}>
                ...
                <td className="whitespace-nowrap px-2.5 py-2.5 text-center align-middle">
                  <button
                    type="button"
                    onClick={(event) => {
                    ...
                    className="inline-flex items-center gap-1 whitespace-nowrap rounded-md border border-[#009E92]/30 bg-[#F0FAF8] px-2 py-0.5 text-[11px] font-medium text-[#007C72] transition hover:bg-[#009E92] hover:text-white"
                    title="View details"
                  >
