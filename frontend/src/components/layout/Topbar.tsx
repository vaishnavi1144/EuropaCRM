...
import { Bell, ChevronDown, LogOut, Mail, Settings, UserCog, HelpCircle, AtSign } from 'lucide-react';
import { useNavigate, useLocation } from 'react-router-dom';
import { useApp, UserRole } from '@/context/AppContext';
import { api } from '@/lib/api';
import { toast } from 'sonner';
import { EmailAccountDialog } from '@/components/crm/EmailAccountDialog';
...
export function Topbar({ onMenu }: { onMenu: () => void }) {
  ...
  useEffect(() => {
    ...
    if (params.get('connected') === 'google') {
      toast.success('Gmail account connected successfully! Outgoing emails will now be sent via your Gmail.');
      const cleanUrl = window.location.pathname;
      window.history.replaceState({}, '', cleanUrl);
    } else if (params.get('error')) {
      toast.error(`Google authentication error: ${params.get('error')}`);
      const cleanUrl = window.location.pathname;
      window.history.replaceState({}, '', cleanUrl);
    }
  }, []);
  ...
  return (
    <header className="crm-topbar fixed right-0 top-0 z-30 h-20 border-b border-[#E4ECF3] bg-white/95 shadow-sm backdrop-blur">
          ...
          <DropdownMenu.Root>
            ...
            <DropdownMenu.Portal>
              <DropdownMenu.Content className="z-50 w-56 rounded-xl border border-[#E4ECF3] bg-white p-1.5 text-[13px] font-semibold text-[#071B4A] shadow-xl" sideOffset={8} align="end">
                ...
                <DropdownMenu.Item onSelect={() => setEmailDialogOpen(true)} className="flex cursor-pointer items-center gap-2 rounded-lg px-2.5 py-2 outline-none hover:bg-slate-50">
                  <AtSign className="h-4 w-4 text-[#009E92]" />
                  <span>Email Account (Gmail)</span>
                </DropdownMenu.Item>
                {hasPermission('users') && (
      ...
      <EmailAccountDialog open={emailDialogOpen} onOpenChange={setEmailDialogOpen} />
    </header>
  );
}
