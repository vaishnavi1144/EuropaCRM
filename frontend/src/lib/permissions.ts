...
export const ROLE_PERMISSION_DEFAULTS: Record<string, string[]> = {
  ...
  SALES: ['dashboard', 'leads', 'contacts', 'accounts', 'opportunities', 'campaigns', 'activities', 'sales-reports', 'sales-mail-view', 'sales-mail-compose', 'sales-mail-bulk', 'sales-mail-groups-view', 'sales-mail-groups-create', 'sales-mail-groups-edit', 'sales-mail-groups-delete'],
  RECRUITER: ['dashboard', 'candidates', 'jobs', 'interviews', 'offers', 'recruitment-reports', 'recruitment-mail-view', 'recruitment-mail-compose', 'recruitment-mail-bulk', 'recruitment-mail-groups-view', 'recruitment-mail-groups-create', 'recruitment-mail-groups-edit', 'recruitment-mail-groups-delete'],
  BENCHSALES: ['dashboard', 'jobs', 'bench', 'submissions', 'placements', 'bench-reports', 'bench-mail-view', 'bench-mail-compose', 'bench-mail-bulk', 'bench-mail-groups-view', 'bench-mail-groups-create', 'bench-mail-groups-edit', 'bench-mail-groups-delete'],
  AITEAM: ['dashboard', 'ai-projects', 'tasks', 'resources', 'ai-reports', 'ai-mail-view', 'ai-mail-compose', 'ai-mail-bulk', 'ai-mail-groups-view', 'ai-mail-groups-create', 'ai-mail-groups-edit', 'ai-mail-groups-delete'],
};
...
export function effectivePermissions(role?: string, permissions?: string[] | null): string[] {
  ...
  const base = Array.isArray(permissions) ? permissions : defaults;

  // Settings & Users are strictly reserved for Admins
  const filtered = base.filter((p) => p !== 'settings' && p !== 'users');
  ...
  if (normalizedRole === 'BENCHSALES') {
    const scoped = filtered.filter((permission) => defaults.includes(permission));
