...
export function ModulePage({ config }: { config: ModuleConfig }) {
  ...
  const openSendResume = async (record: RecordRow) => {
    ...
    const targetEmail = submissionTarget === 'Hiring / End Client'
      ? String(record.targetEmail ?? record.endClientEmail ?? record.hiringClientEmail ?? record.clientEmail ?? '')
...
Europa CRM Team`,
  ...
  const generateVendorEmailHtml = (selectedRows: RecordRow[]) => {
    ...
    const html = `
      <div style="font-family:Arial,sans-serif;color:#1f2937;line-height:1.6;">
        ...
        <br />
        <p>Thanks &amp; Regards,<br /><strong>${String(currentUser?.name ?? 'Bench Sales Recruiter')}</strong><br />${currentUser?.role ? `${currentUser.role.replace(/_/g, ' ')} | ` : ''}Europa CRM${currentUser?.email ? `<br />${currentUser.email}` : ''}</p>
      </div>`;
    return html;
  };
  ...
  const openBulkSendResume = async (selectedRows: RecordRow[]) => {
    ...
    const targetEmails = getSelectedRowEmail(resolvedRows, ['targetEmail', submissionTarget === 'Hiring / End Client' ? 'endClientEmail' : 'vendorEmail', 'hiringClientEmail', 'recruiterEmail', 'contactEmail']);
...
${entry.match.hardBlockers.join('\n')}`); return; }
  ...
  return (
    <div className="px-5 py-5">
      ...
      <RecordDialog
        key={`${config.resource}_${editing ? editing.id : 'new'}_${dialogOpen}`}
        open={dialogOpen}
      ...
      <EmailDialog
        ...
        onSend={async (payload) => {
          const getModule = (res: string): 'sales' | 'it' | 'bench' | 'ai' => {
            ...
            if (['jobs', 'bench', 'submissions', 'requirements', 'bench-interviews', 'outcome-feedback', 'bench-offers', 'placements'].includes(res)) return 'bench';
            return 'ai';
          };
