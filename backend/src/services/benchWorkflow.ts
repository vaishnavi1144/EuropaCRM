...
export async function enrichAndValidateBenchInput(
  ...
  if (resource === 'submissions') {
    ...
    merged.vendorName = text(merged.vendorName) || text(jobCustom.vendorName) || '';
    ...
    merged.targetEmail = merged.submissionTarget === 'Hiring / End Client'
      ? String(jobCustom.endClientEmail ?? jobCustom.hiringClientEmail ?? '')
      : (job.vendorEmail || '');
