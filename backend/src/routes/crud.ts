...
const resources = {
  ...
  'outcome-feedback': { model: 'outcomeFeedback', type: 'OutcomeFeedback', search: ['candidateName', 'jobTitle', 'status', 'result', 'ownerName', 'technicalFeedback', 'communicationFeedback', 'feedback'] },
...
const resourcePermission: Partial<Record<ResourceName, string>> = {
  ...
  bench: 'bench', submissions: 'submissions', 'submission-emails': 'submissions', requirements: 'submissions', 'bench-interviews': 'submissions', 'outcome-feedback': 'submissions', 'bench-offers': 'placements', placements: 'placements',
...
crudRouter.post('/:resource', async (req, res, next) => {
  try {
    ...
    const result = await prisma.$transaction(async (tx: any) => {
      ...
      validateCoreResourceFields(resource, source as Record<string, unknown>);
      const created = await tx[config.model].create({ data: await prepareData(config.type, source) });
      if (resource === 'outcome-feedback' && created.interviewId) {
        try {
          await tx.benchInterview.update({
            where: { id: created.interviewId },
            data: {
              ...(created.status ? { status: created.status } : {}),
              ...(created.result ? { result: created.result } : {}),
              ...(created.feedbackDate ? { feedbackDate: created.feedbackDate } : {}),
              ...(created.feedback ? { feedback: created.feedback } : {}),
              ...(created.rejectionStage ? { rejectionStage: created.rejectionStage } : {}),
              ...(created.rejectionReason ? { rejectionReason: created.rejectionReason } : {}),
            }
          });
        } catch (_) {}
      }
      ...
      const workflowEvents = BENCH_WORKFLOW_RESOURCES.has(resource)
        ? await applyBenchAutomation(tx, resource, synchronized, null, res.locals.authUser ?? {})
        : [] as WorkflowEvent[];
      const latest = await tx[config.model].findUnique({ where: { id: created.id } });
...
crudRouter.put('/:resource/:id', async (req, res, next) => {
  try {
    const resource = req.params.resource as ResourceName;
    const config = resources[resource];
    if (resource === 'placements') return res.status(409).json({ message: 'Update Placements directly only for non-commercial operational fields.' });
    if (resource === 'users' && String(req.body?.role ?? '').toUpperCase() === 'SUPER_ADMIN' && String(res.locals.authUser?.role ?? '') !== 'SUPER_ADMIN') {
    ...
    const result = await prisma.$transaction(async (tx: any) => {
      ...
      if (resource === 'outcome-feedback' && updated.interviewId) {
        try {
          await tx.benchInterview.update({
            where: { id: updated.interviewId },
            data: {
              ...(updated.status ? { status: updated.status } : {}),
              ...(updated.result ? { result: updated.result } : {}),
              ...(updated.feedbackDate ? { feedbackDate: updated.feedbackDate } : {}),
              ...(updated.feedback ? { feedback: updated.feedback } : {}),
              ...(updated.rejectionStage ? { rejectionStage: updated.rejectionStage } : {}),
              ...(updated.rejectionReason ? { rejectionReason: updated.rejectionReason } : {}),
            }
          });
        } catch (_) {}
      }
